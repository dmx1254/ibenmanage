...
import { deleteOrderVente } from "@/lib/actions/patient.actions";

const OrderVenteDel = ({ id }: { id: string }) => {
  ...
  const handleDeleteVente = async () => {
    try {
      setIsDeleting(true);
      const orderDeleted = await deleteOrderVente(id);
      if (orderDeleted) {
        toast.success(
          `La commande N° ${orderDeleted.orderNum} à été supprimé avec succès`,
          {
            style: {
              color: "#22c55e",
              background: "#0D0F10",
