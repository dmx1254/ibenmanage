...
import { deleteServeurAchat } from "@/lib/actions/patient.actions";
import { toast } from "sonner";

const DeleteServerAchat = ({ id }: { id: string }) => {
  ...
  const handleDeletePatient = async (userId: string) => {
    try {
      setIsDeleting(true);
      const serverDeleted = await deleteServeurAchat(userId);
      if (serverDeleted) {
        toast.success(
...
export default DeleteServerAchat;
