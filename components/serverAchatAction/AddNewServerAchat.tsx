...
import { createAchatServer } from "@/lib/actions/patient.actions";
import { toast } from "sonner";

const AddNewServerAchat = () => {
  ...
  const handleUpdateServerAchat = async () => {
    if (
      ...
      Number(formdata.serverPriceDh) < 0 ||
      ...
      try {
        setIsLoading(true);
        const response = await createAchatServer(formdata);
        if (response) {
            ...
            <AlertDialogDescription className="my-4">
              <div className="grid grid-cols-1 md:grid-cols-2">
                ...
                <div className="mt-4 relative">
                  <Label htmlFor="serverPriceDh">Prix</Label>
                  <Input
                    type="number"
                    value={
                      formdata.serverPriceDh > 0 ? formdata.serverPriceDh : ""
                    }
                    name="serverPriceDh"
                    id="serverPriceDh"
                    placeholder="0"
                    onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      handleChangeFormdata("serverPriceDh", e.target.value)
                    }
                    className="bg-transparent mt-2 border-dark-500 text-white/80"
                  />
...
export default AddNewServerAchat;
