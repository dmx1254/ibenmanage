...
import { serverAchatUp } from "@/lib/actions/patient.actions";
import { toast } from "sonner";
import clsx from "clsx";

const UpdateServerAchat = ({
  data,
  type,
}: {
  data: ServerSell;
  ...
  const handleUpdateServerAchat = async () => {
    try {
      setIsLoading(true);
      const response = await serverAchatUp(data._id, formdata);
      if (response) {
        toast.success(
...
export default UpdateServerAchat;
