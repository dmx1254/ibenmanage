...
const OrderBuyDel = ({ id }: { id: string }) => {
  ...
  const handleDeleteDoctor = async (patientId: string) => {
    try {
      setIsDeleting(true);
      const orderDeleted = await deleteOrderBuy(patientId);
      if (orderDeleted) {
        toast.success(
          `La commande N° ${orderDeleted.numBuy} à été supprimé avec succès`,
          {
            style: {
              color: "#22c55e",
              background: "#0D0F10",
        ...
        <AlertDialogContent className="bg-dark-200 border-dark-300 w-full">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="max-sm:text-sm">
              Cette action est irréversible. Cela supprimera définitivement cette commande.
            </AlertDialogDescription>
          </AlertDialogHeader>
